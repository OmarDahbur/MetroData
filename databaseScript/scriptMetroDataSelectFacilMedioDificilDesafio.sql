...
SELECT
IFNULL(e.nome, 'Estação desconhecida') AS nome_estacao,
COUNT(o.idOcorrencia) AS total_ocorrencias,
AVG(u.idade) AS media_idade_usuarios,
CASE
WHEN AVG(u.idade) < 25 THEN 'Jovens'
WHEN AVG(u.idade) BETWEEN 25 AND 50 THEN 'Adultos'
ELSE 'Idosos'
END AS faixa_etaria_media
FROM estacao e
RIGHT JOIN ocorrencia o ON e.idEstacao = o.fkEstacao
JOIN usuario u ON o.fkUsuario = u.idUsuario
GROUP BY e.idEstacao
HAVING COUNT(o.idOcorrencia) > 1
ORDER BY media_idade_usuarios ASC;
...
MAX(e.nome) AS nome_estacao, -- Exibir uma estação associada (mais recente)
MAX(e.zona) AS zona,
MAX(e.mediaAtrasosMin) AS media_atrasos_min,

COUNT(DISTINCT o.idOcorrencia) AS total_ocorrencias,
MAX(v.tempoViagemMin) AS viagem_mais_longa,
MIN(b.valor) AS menor_valor_bilhete,
ROUND(AVG(e.passageiros), 0) AS media_passageiros,

MAX(r.statusRelacionamento) AS status_relacionamento,
MAX(r.nivelProximidade) AS nivel_proximidade

FROM usuario u
LEFT JOIN ocorrencia o ON u.idUsuario = o.fkUsuario
LEFT JOIN estacao e ON o.fkEstacao = e.idEstacao
LEFT JOIN bilhete b ON u.idUsuario = b.fkUsuario
LEFT JOIN viagemUsuario vu ON u.idUsuario = vu.fkUsuario
LEFT JOIN viagem v ON vu.fkViagem = v.idViagem
LEFT JOIN relacionamento r ON u.idUsuario = r.fkConjuge1 OR u.idUsuario = r.fkConjuge2

GROUP BY
u.idUsuario, u.nome, u.idade, u.sexo, u.tipoUsuario
